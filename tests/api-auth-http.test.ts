...
test('apiRequest formats validation detail arrays from backend', async () => {
  setFetchMock(async () =>
    new Response(
      JSON.stringify({
        detail: [
  ...
  await assert.rejects(() => apiRequest('register'), /username: String should have at least 3 characters/)
})
